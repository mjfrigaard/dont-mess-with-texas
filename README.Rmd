...
We can see the `Link` column is repeated, which is going to be a problem when we put these data into their own `tibble` (R doesn’t like to repeat the column names inside a `data.frame`).
...
So we will create some new names for this table in a `new_colnames` vector.
...
And use the handy `magrittr::set_colnames()` function to replace the names.
...
Another option is to use `tibble::as_tibble()` function, but with the `.name_repair` argument. This has five options: `"assert_unique"`, `"unique"`, `"syntactic"`, `"none"`, and `"minimal"`. You can research each of these using `?as_tibble`. In this case, `"unique"` or `"syntactic"` will work just fine.
